...
const (
	nKV = iota  //nodeData 表示 KV offset的下标
	nKey		//key的长度的下标
	nVal        //value长度的下标
	nHeight     //当前节点的高度
	nNext       //每一层下一个节点的信息
...
type DB struct {
	...
	//skip list中的update数组,更新、删除操作时记录每层 < key时最右边的位置
...
func (p *DB) findGE(key []byte, prev bool) (int, bool) {
	...
	for {
		next := p.nodeData[node+nNext+h]  //start offset of next node
		cmp := 1
		if next != 0 {
			o := p.nodeData[next] //start offset of key value offset
...
/*
即使key已经存在skip list中，依然往后追加数据，而不是覆盖。
这样是为了可以并发的读？
 */
func (p *DB) Put(key []byte, value []byte) error {
	...
	p.kvData = append(p.kvData, value...)
	// Node
	node := len(p.nodeData) //新增节点的offset
	p.nodeData = append(p.nodeData, kvOffset, len(key), len(value), h)
	for i, n := range p.prevNode[:h] {
		m := n + nNext + i //m指向skip list中第i层位于当前节点前面的按个节点的next数组的第i个位置(i层)
		p.nodeData = append(p.nodeData, p.nodeData[m])  //当前节点的next[i] = m
		p.nodeData[m] = node //修复m指向当前节点
	}
